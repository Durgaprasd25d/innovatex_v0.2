...
const RegistrationForm = () => {
  ...
  const [collegeName, setCollegeName] = useState("") // College name at team level
  const [teamMembers, setTeamMembers] = useState<TeamMember[]>([{ name: "", email: "", contactNo: "", gender: "" }])
  ...
  const handleAddMember = () => {
    if (teamMembers.length < 5) {
      setTeamMembers([...teamMembers, { name: "", email: "", contactNo: "", gender: "" }])
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!teamName || !selectedTrack || !collegeName || !paymentScreenshot) {
    ...
    const isAllMembersValid = teamMembers.every(
      (member) => member.name && member.email && member.contactNo && member.gender,
    )
    ...
    formData.append("collegeName", collegeName) // Add college name to form data
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTeamName("")
        setSelectedTrack("")
        setCollegeName("") // Reset college name
        ...
        <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="bg-gray-900 p-4 sm:p-6 md:p-8 rounded-lg shadow-lg border border-gray-700">
              <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 sm:mb-8">Team Registration</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                {/* Team Name */}
                <div>
                  <label className="block text-blue-400 text-sm sm:text-base mb-2">Team Name</label>
                  <input
                    type="text"
                    value={teamName}
                ...
                {/* College Name - Added at team level */}
                <div>
                  <label className="block text-blue-400 text-sm sm:text-base mb-2">College Name</label>
                  <input
                    type="text"
                    value={collegeName}
                ...
                {/* Track Selection */}
                <div>
                  <label className="block text-blue-400 text-sm sm:text-base mb-2">Select Track</label>
                  <select
                    value={selectedTrack}
                ...
                {/* Team Members */}
                <div>
                  <label className="block text-blue-400 text-sm sm:text-base mb-2">Team Members (Max 5)</label>
                ...
                <div>
                  <label className="block text-blue-400 text-sm sm:text-base mb-2">Upload Payment Screenshot</label>
                  <div className="border-2 border-dashed border-gray-700 rounded-lg p-4 sm:p-6 text-center">
                    ...
                    {!previewURL ? (
                      ...
                      >
                        ...
                        <span className="text-xs sm:text-sm text-gray-400">Click to upload payment screenshot</span>
                      </label>
                    ) : (
